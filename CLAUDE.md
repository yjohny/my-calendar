...
## Architecture
- **Models/CalendarStore.swift** — Main `@Observable` store. Manages `dayTexts` (full interleaved text per date — events and journal mixed in user's order) and `eventColorMap` (calendar colors from EventKit keyed by title+time). Parses user text, syncs events to EventKit, stores full text to file preserving layout order. Events and journal text can be freely interleaved. Exposes `syncStatus` (`.idle`, `.saving`, `.syncing`) for UI feedback.
...
- **DateScrubber caching**: Month tick positions are computed as normalized progress values (0…1) and cached; they're only recomputed when `startDate`/`endDate` change. The `DateFormatter` for the floating label is a static cached instance. Haptic feedback (light impact) fires when dragging across month boundaries.
- **StyledTextView**: Uses concatenated `Text` views (not `HStack`) so long event names wrap to the next line naturally.
- **Toolbar**: Today, Search, Calendars, and Help are in a standard iOS bottom toolbar (`NavigationStack` + `ToolbarItemGroup(.bottomBar)`). A sync status indicator (spinner + "Saving..."/"Syncing...") appears in the navigation bar's principal slot during persistence operations.
...
- **LRU cache for dayTexts** — `dayTexts` dictionary grows unbounded as the user scrolls. Could evict old entries and reload from file on demand.
