...
- **Persistence/** — `EventKitManager` (actor) wraps EKEventStore CRUD; removal methods throw on failure. `EventKitSync` converts between text lines and `EKEvent` objects. `FileStore` (actor) handles per-day text files. `ChangeCoalescer` debounces saves and reports errors via an `onError` callback.
...
- **Debounced EventKit sync**: `CalendarStore.update()` debounces EventKit sync (800ms) via a cancellable `syncTask` so typing doesn't trigger a delete-and-recreate cycle on every keystroke. File saves are separately debounced (500ms) via `ChangeCoalescer`.
...
- **StyledTextView**: Uses concatenated `Text` views (not `HStack`) so long event names wrap to the next line naturally. Applies `ensureContrast()` to EventKit calendar colors to ensure readability in both light and dark mode.
...
- **Error propagation**: `ChangeCoalescer` reports save errors via its `onError` callback. `EventKitManager.removeEvent()` and `removeManagedEvents()` throw on failure. `CalendarStore` catches these and sets `syncStatus = .error(...)` for the UI to display. Never silently swallow save/sync errors with `try?`.
- **LRU cache**: `CalendarStore` tracks date access order and evicts entries beyond 180 days. On cache miss, `displayText(for:)` lazy-loads from `FileStore`. When adding LRU eviction, also evict `eventColorMap`, `unmatchedEventLines`, `journals`, and `parsedKeyCache` entries for the evicted date.
